...
class EKF(object):
    ...
    def update(self, u, z, Q, R, dyn_func_params=None, meas_func_params=None):
        ...
        # Predict measurement
        z_pred, H = self.measurement(mu_pred, params=meas_func_params, jac=True)
        ...
        self.mu = mu_pred + np.matmul(K, z - z_pred)
        self.S = S_pred - np.matmul(K, np.matmul(H, S_pred))
